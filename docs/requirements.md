...
AndroidのWireless Debuggingでは接続ポートが固定ではないため、MVPでは設定画面に表示されるポート番号をユーザーが入力する。毎回の再入力の負担を減らすため、接続用ポートだけはアプリ内に永続化し、次回起動時に復元する。ペアリング用ポートはペアリングダイアログを開くたびに値が変わるため永続化しない。6桁のペアリングコードは引き続き一切永続化しない。
...
画面とポート入力・取得値は`ViewModel`で保持し、回転で失わない。接続用ポートのみアプリのプロセス再起動をまたいで永続化し、起動時に検証済みの値として復元する。復元値が不正なら空文字として扱う。ペアリング用ポートは永続化しない。ペアリングコードは保存せず、接続設定を離れると破棄する。「ADB認証情報をリセット」を実行すると、永続化した接続用ポートも削除する。
...
アプリ内に「ADB資格情報を削除」操作を用意し、Keystore aliasとfallback暗号文を削除できるようにする。この操作では、永続化した接続用ポートも合わせて削除し、完全にまっさらな状態から再ペアリングできるようにする。
...
- 接続用ポートの永続化: 接続成功時のみ保存され、接続失敗時は保存されないこと。復元した値が不正な場合は空文字として扱われること
